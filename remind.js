...
exports.checkReminders = function(client) {
var now = Date.now();
Object.keys(reminders).map(date=>{
    if (date <= now) {
        //var recipent = client.users.get('id', reminders[date].user);
        var recipent = client.users.get(reminders[date].user);
        if (recipent) recipent.send("⏰ **Reminder:** "+reminders[date].text);
        console.log(" DEBUG  Reminded user");
...
Object.keys(reminders).map(t=>{
    ...
    if (reminders[t].user == user && reminders[t].text.indexOf(text) > -1) {
        delete reminders[t];
        updatedR = true;
        if (debug) console.log(" DEBUG  Removed reminder for user " + user);
        found = true;
    }
});
